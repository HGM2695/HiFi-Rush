...
namespace gm
{
	...
	public:
		...
		void				RemoveNotifyCallback(int id);
		void				ClearNotifyCallbacks() { _notifyCallbacks.clear(); }
	...
	private:
		...
		{
			int id = -1;
			AnimationNotifyCallback callback;
		};

		std::vector<NotifyCallbackEntry>		_notifyCallbacks;
		float									_previousTime = 0.f;
		int										_nextNotifyCallbackId = 0;
		// NotifyConnection이 디스패처 생존 여부를 확인할 수 있도록 shared lifetime token을 유지합니다.
		std::shared_ptr<void>					_lifetimeToken;
	};

	// BindNotifyCallback 함수가 반환한 NotifyConnection이 소멸될 때,
	// 디스패처에 등록된 콜백이 자동으로 해제됩니다.
	class NotifyConnection
	{
	public:
